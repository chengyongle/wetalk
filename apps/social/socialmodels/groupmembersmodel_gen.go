...
type (
	groupMembersModel interface {
		...
		Update(ctx context.Context, session sqlx.Session, data *GroupMembers) error
	...
	GroupMembers struct {
		...
		MemberStatus int64          `db:"member_status"` // 状态 1:在群 2:退群
...
func (m *defaultGroupMembersModel) Insert(ctx context.Context, session sqlx.Session, data *GroupMembers) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, data.GroupId, data.UserId, data.RoleLevel, data.MemberStatus, data.JoinTime, data.JoinSource, data.InviterUid, data.OperatorUid)
		}
		return conn.ExecCtx(ctx, query, data.GroupId, data.UserId, data.RoleLevel, data.MemberStatus, data.JoinTime, data.JoinSource, data.InviterUid, data.OperatorUid)
	}, groupMembersIdKey)
	return ret, err
}

func (m *defaultGroupMembersModel) FindByGroudIdAndUserId(ctx context.Context, userId, groupId string) (*GroupMembers, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `group_id` = ? and `member_status` = 1", groupMembersRows, m.table)
...
func (m *defaultGroupMembersModel) ListByUserId(ctx context.Context, userId string) ([]*GroupMembers, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `member_status` = 1", groupMembersRows, m.table)
...
func (m *defaultGroupMembersModel) ListByGroupId(ctx context.Context, groupId string) ([]*GroupMembers, error) {
	query := fmt.Sprintf("select %s from %s where `group_id` = ? and `member_status` = 1", groupMembersRows, m.table)
...
func (m *defaultGroupMembersModel) Update(ctx context.Context, session sqlx.Session, data *GroupMembers) error {
	groupMembersIdKey := fmt.Sprintf("%s%v", cacheGroupMembersIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, groupMembersRowsWithPlaceHolder)
		return session.ExecCtx(ctx, query, data.GroupId, data.UserId, data.RoleLevel, data.MemberStatus, data.JoinTime, data.JoinSource, data.InviterUid, data.OperatorUid, data.Id)
	}, groupMembersIdKey)
	return err
}
